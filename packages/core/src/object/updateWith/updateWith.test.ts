...
import { isObject, noop } from '@/is';
import { describe, expect, it } from 'vitest';
import { toString, unset } from '../../';
...
describe('updateWith', () => {
  ...
  it('should prevent prototype pollution by skipping __proto__ in path', () => {
    const object = { a: [{ ['__proto__']: { a: 0, b: 3 } }] };
  ...
  it('should work with a `customizer` callback', () => {
    const actual = updateWith(
      { 0: {} },
      '[0][1][2]',
      () => 3,
      value => (isObject(value) ? undefined : {}),
    );
  ...
  it('should work with a `customizer` that returns `undefined`', () => {
    const actual = updateWith({}, 'a[0].b.c', () => 4, noop);
