...
import type { BoxStyle, Boxes } from 'cli-boxes';
...
export type Styles = {
  ...
  /**
   * Sets a background color of the element.
   ...
   *
   * Use it with at least one size constraint (`width`, `height`, `minHeight`, or `maxHeight`) so the ratio can derive the missing dimension.
   */
   ...
   * Shorthand for setting `borderTopBackgroundColor`, `borderRightBackgroundColor`, `borderBottomBackgroundColor` and `borderLeftBackgroundColor`.
   */
...
const applyFlexStyles = (node: YogaNode, style: Styles): void => {
  ...
  if ('alignContent' in style) {
    // Falsy must agree with `resetYogaStyles` (`DOMElement.ts`), which starts
    // elements at `ALIGN_STRETCH` rather than Yoga's own `FLEX_START` default.
    // Reachable: `patchProp` (`src/nodeOps.ts`) removes an attribute only for a
    // strictly `undefined` value, so `align-content=""` arrives here. And
    // load-bearing, because styles are now reset and reapplied only when an
    // element's attributes change — this branch is what restores the stretch
    // default when `alignContent` is withdrawn to a falsy value.
