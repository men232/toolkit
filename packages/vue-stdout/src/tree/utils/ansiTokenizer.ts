/**
 * A minimal ANSI/C1 control-sequence tokenizer.
 *
 * Ported from ink's `ansi-tokenizer.ts`. `@alcalzone/ansi-tokenize` (already a
 * dependency, used by `Layer` to build the styled-character grid) is not a
 * substitute here: it is built to turn a styled string into character cells,
 * and asked to tokenize a lone screen-clear plus trailing text it lumps
 * everything from the escape onward into one `ansi` token, swallowing text
 * that follows. `sanitizeAnsi` needs to classify *which* escape sequence it is
 * looking at (SGR vs. everything else) without losing surrounding text, which
 * means implementing the actual CSI/OSC grammar rather than approximating it
 * with a regular expression.
 *
 * Standards references:
 * - ECMA-48 control functions and CSI byte classes:
 *   https://ecma-international.org/publications-and-standards/standards/ecma-48/
 * - xterm CSI parameter/intermediate/final format notes:
 *   https://invisible-island.net/xterm/ecma-48-parameter-format.html
 * - xterm/OSC BEL termination behaviour:
 *   https://davidrg.github.io/ckwin/dev/ctlseqs.html
 */
