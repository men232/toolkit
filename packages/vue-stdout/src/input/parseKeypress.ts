// Ported from ink 7.1.1's `src/parse-keypress.ts`, which itself is copied
// from https://github.com/enquirer/enquirer/blob/36785f3399a41cd61e9d28d1eb9c2fcd73d69b4c/lib/keypress.js
//
// This is a pure function over strings/bytes: no terminal, no state. Its
// correctness is verified differentially against ink's own build output in
// `parseKeypress.test.ts` (see `test/helpers/inkParser.ts`).
...
// Kitty keyboard protocol flags/modifiers.
...
const parseKittyKeypress = (s: string): Key | null => {
...
const parseKittySpecialKey = (s: string): Key | null => {
...
export const parseKeypress = (s: Uint8Array | string = ''): Key => {
  let parts;
