// Ported from ink 7.1.1's `src/input-parser.ts`.
//
// This is the stream-level layer sitting above `parseKeypress`: it turns
// raw chunks read off stdin into discrete input events, because a single
// `read()` can carry zero, one, or several complete escape sequences, and
// any sequence can be split across two reads. It never calls `parseKeypress`
// itself (ink calls that further downstream, per emitted event) — this
// module only knows how to find sequence boundaries in a string.
//
// Pure function over strings: no terminal, no raw-mode side effects. Its
// correctness is verified differentially against ink's own build output in
// `inputParser.test.ts` (see `test/helpers/inkParser.ts`).
//
// Buffer-ownership note: unlike `parseKeypress`, this module never touches a
// `Uint8Array` and never mutates its input. `push()` only ever does
// `pending + chunk` (string concatenation, which allocates a new string) and
// `input.slice(...)` (which also allocates). JS strings are immutable, so
// there is nothing here for a shared buffer to corrupt. The mutation hazard
// documented for `parseKeypress` (a `Uint8Array` with a lone high-bit byte
// gets decremented in place) belongs to whatever downstream code eventually
// feeds each emitted event string into `parseKeypress` — out of scope for
// this module, since it never constructs a `Uint8Array` and never calls
// `parseKeypress`.
